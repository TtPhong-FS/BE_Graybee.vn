...
public interface FavouriteRepository extends JpaRepository<Favorite, Integer> {
    ...
    @Query("Select new vn.graybee.modules.account.dto.response.FavoriteProductResponse(p.id, p.price, p.finalPrice, p.name, p.thumbnail) from Favorite f join Product p on f.productId = p.id where f.accountId = :accountId")
