...
public interface CategoryRepository extends JpaRepository<Category, Long> {
    ...
    @Query("Select new vn.graybee.modules.catalog.dto.response.CategoryUpdateDto(c.name, (Select p.name from Category p where p.id = c.parentId), c.slug, c.type, c.status) from Category c where c.id = :id")
    Optional<CategoryUpdateDto> findCategoryUpdateDtoById(Long id);

    @Query("Select new vn.graybee.modules.catalog.dto.response.CategorySummaryDto(c.id, c.name, c.type, c.status) from Category c where c.name = :name or c.id = :id")
    ...
    @Query("Select new vn.graybee.modules.catalog.dto.response.CategorySummaryDto(c.id, c.name, c.type, c.status) from Category c where c.name in :names")
