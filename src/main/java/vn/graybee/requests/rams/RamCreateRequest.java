package vn.graybee.requests.rams;
...
public class RamCreateRequest {
    ...
    @NotNull(message = "Ram productId cannot be null")
    @PositiveOrZero(message = "ProductId cannot be negative")
    private long productId;

    @NotEmpty(message = "Ram's ram type cannot be empty")
    @Size(min = 2, max = 5, message = "Ram Type must be between 2 and 5 characters")
    @JsonProperty("ram_type")
    private String ramType;

    @NotEmpty(message = "Ram's series cannot be empty")
    @Size(min = 0, max = 100, message = "Ram Type must be between 0 and 100 characters")
    ...
    @Size(min = 0, max = 50, message = "Latency must be between 0 and 50 characters")
    private String latency;

    @JsonProperty("voltage")
    private float voltage = 0.5F;
