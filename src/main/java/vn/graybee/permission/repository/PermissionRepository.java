...
import vn.graybee.permission.dto.response.PermissionBasicDto;
...
public interface PermissionRepository extends JpaRepository<Permission, Integer> {
    ...
    @Query("Select new vn.graybee.permission.dto.response.PermissionUserCountDto(p.id, p.userCount) from Permission p where p.id = :id")
    ...
    @Query("Select new vn.graybee.permission.dto.response.PermissionBasicDto(p.id, p.name) from Permission p where p.id IN :ids")
