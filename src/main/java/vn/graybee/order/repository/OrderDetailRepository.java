...
public interface OrderDetailRepository extends JpaRepository<OrderDetail, Long> {
    ...
    @Query("SELECT new vn.graybee.order.dto.response.user.OrderMapOrderDetailWithProductBasicDto(od.orderId, od.id,p.id, p.name, p.thumbnail, od.quantity, od.subtotal, od.priceAtTime) " +
    ...
    @Query("SELECT new vn.graybee.order.dto.response.user.OrderTotalQuantityDto(od.orderId, SUM(od.quantity)) FROM OrderDetail od WHERE od.orderId IN :orderIds GROUP BY od.orderId")
