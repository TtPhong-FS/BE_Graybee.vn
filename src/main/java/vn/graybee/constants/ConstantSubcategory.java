package vn.graybee.constants;

public class ConstantSubcategory {

    public static final String general = "general";

    public static final String name = "name";

    public static final String subcategories = "subcategories";

    public static final String name_exists = "Tên danh mục con đã tồn tại trong hệ thống";

    public static final String does_not_exists = "Danh mục con không tồn tại trong hệ thống";

    public static final String tag_relation_does_not_exists = "Thẻ phân loại hiện chưa được liên kết với danh mục con";


    //    Response
    public static final String success_create = "Tạo danh mục con thành công";

    public static final String success_update_by_id = "Cập nhật danh mục con thành công";

    public static final String success_find_by_id = "Tìm danh mục con thành công";

    public static final String success_fetch_subcategories = "Danh sách danh mục con đã được lấy thành công";

    public static final String success_delete_by_id = "Xoá danh mục con thành công";

    public static final String success_delete_tag_relation = "Thẻ phân loại đã được loại bỏ khỏi danh mục con";

}
